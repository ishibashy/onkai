...
int zennon[8] = {262, 294, 330, 350, 392, 440, 494, 524}; //全音2オクターブ分
int hannon[6] = {278, 312, 370, 416, 466, 554}; 
int countZEN = 0;
int countHAN = 0;
int toneDelay = 100;
...
void setup() {
  ...
  Serial.println(F("Hello!"));
    tone(PINNO, (zennon[countZEN + 4])*2) ;
    delay(400);
    tone(PINNO, (zennon[countZEN + 3])*2) ;
    delay(200);
    tone(PINNO, (zennon[countZEN + 7])*2) ;
    delay(1000);
    noTone(PINNO);
}
//-----メイン-----
...
void loop() {
  ...
  if (digitalRead(9) == HIGH) {//ド(↑)
    ...
    delay(toneDelay);
    noTone(PINNO);
  } else if (digitalRead(10) == HIGH) {//音階を1オクターブ左にシフト
...
//      countZEN = 7;
    ...
    delay(500);
  } else if (digitalRead(12) == HIGH) {//音階を1オクターブ右にシフト
...
//  for(int i = 0; i < 8; i++){
//    Serial.print(zennon[i]);
//    Serial.print(", ");
//  }
//  Serial.print(F("\t"));
//  for(int i = 0; i < 6; i++){
//    Serial.print("   ");
//    Serial.print(hannon[i]);
//    Serial.print(",");
//    Serial.print(F("\t"));
//  }
//  Serial.print("\n");
